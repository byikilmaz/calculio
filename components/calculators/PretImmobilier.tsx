...
import {
  computePretImmobilierFR,
  type PretImmobilierResult,
} from "@/lib/calculators/simulateur-pret-immobilier-fr";
...
export function PretImmobilierCalculator() {
  ...
  const [assurance, setAssurance] = useState<string>("0.34");
  const [showTable, setShowTable] = useState<boolean>(false);

  const result = useMemo<PretImmobilierResult | null>(() => {
    ...
    return computePretImmobilierFR({
      montant: m,
      dureeAnnees: d,
      tauxAnnuel: t,
      tauxAssurance,
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-slate-500">
            Renseignez montant, durée, taux pour obtenir vos mensualités
          </p>
        </CardHeader>
        <CardBody className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <Label htmlFor="assurance">Taux assurance emprunteur</Label>
              <div className="relative">
                <Input
                  id="assurance"
                  inputMode="decimal"
      ...
      <Card>
        ...
        <CardBody className="space-y-3 text-sm">
          <Row
            ...
            value={formatEuro(result.montant)}
          />
          <Row label="Durée" value={`${result.dureeAnnees} ans (${result.dureeMois} mois)`} />
          <Row
